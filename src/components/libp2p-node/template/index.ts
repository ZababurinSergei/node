/**
 * Шаблоны для компонента Libp2p Node
 */
...
export function renderLogsPart({ state = {} }: { state: any; context?: any }): string {
    const { logs = [] } = state;
...
export function renderMultiaddrsPart({ state = {} }: { state: any; context?: any }): string {
...
export function renderConnectionsPart({ state = {} }: { state: any; context?: any }): string {
