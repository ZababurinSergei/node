/**
 * Шаблоны для DHT Manager компонента
 */

export function defaultTemplate({ state = {}, context }: { state: any; context: any }): string {
    return `
...
export function renderDHTStats({ state = {} }: { state: any; context?: any }): string {
    const { dhtStats, activeDHTType } = state;

    let instancesToShow: string[] = [];
...
export function renderBucketInfo({ state = {} }: { state: any; context?: any }): string {
    ...
    return `
        ...
        bucketInfo.buckets.map((bucket: any) => `
...
export function renderAPIResponse({ state = {} }: { state: any; context?: any }): string {
    ...
    return `
        <div class="api-response-container ${isSuccess ? 'api-response-success' : 'api-response-error'}">
            <div class="api-response-header">
                ...
                <span class="response-timestamp">${new Date().toLocaleTimeString()}</span>
            </div>
...
function renderDHTInstance(type: string, stats: any): string {
    if (!stats) {
        return `
    ...
    const statusText = stats.status === 'running' ? 'Running' :
        stats.status === 'stopped' ? 'Stopped' :
            stats.status === 'error' ? 'Stopped' : 'Stopped';

    // ОБЕСПЕЧИВАЕМ КОРРЕКТНОЕ ОТОБРАЖЕНИЕ МЕТРИК
    const successRate = stats.metrics && stats.metrics.successRate !== undefined
        ? typeof stats.metrics.successRate === 'number'
            ? `${stats.metrics.successRate.toFixed(1)}%`
            : stats.metrics.successRate === 0
                ? '0%'
                : `${stats.metrics.successRate}%`
        : '0%';

    const queryLatency = stats.metrics && stats.metrics.queryLatency ?
        (typeof stats.metrics.queryLatency === 'number'
            ? stats.metrics.queryLatency + 'ms'
            : stats.metrics.queryLatency === 0
                ? '0ms'
                : stats.metrics.queryLatency)
        : '0ms';

    const bucketsInfo = stats.buckets ?
        `${stats.buckets.full || 0}/${stats.buckets.total || 0} full` : '0/0 full';
