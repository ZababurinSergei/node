...
export async function createActions(context: DHTManager) {
    return {
 ...
 */
function createEmptyDHTStats(type: string, status = 'stopped'): any {
    return {
        type: type,
        peerCount: 0,
        routingTableSize: 0,
...
/**
 * Загружает статистику DHT из libp2p-node
 */
async function loadDHTStats(this: DHTManager, data: any): Promise<void> {
    try {
        console.log('🔄 Loading DHT stats with data:', data);

        // ЗАЩИТА ОТ UNDEFINED - ключевое исправление
        if (!data) {
            console.warn('⚠️ DHT stats data is undefined, using default structure');
            data = { stats: null };
        }

        // Получаем DHT статистику из libp2p-node
        ...
        if (response.success && response.dhtStats) {
            // Используем postMessage для обновления состояния
        ...
        // СОЗДАЕМ СТРУКТУРУ ПО УМОЛЧАНИЮ ПРИ ОШИБКЕ
        await this.postMessage({
            type: 'UPDATE_DISPLAY',
            data: {
                stats: {
...
/**
 * Выполняет поиск пира в DHT через libp2p-node
 */
async function findPeer(this: DHTManager, peerId: string, dhtType = 'all'): Promise<DHTResponse> {
    ...
    try {
        ...
        // Получаем libp2p-node
        const libp2pNode = await this.getComponentAsync('libp2p-node', 'libp2p-node-1');
        if (!libp2pNode) {
            throw new Error('libp2p-node not found. Please start the node first.');
        }

        // Используем postMessage для поиска пира через libp2p-node
        ...
        if (response.success && response.result) {
            ...
            const foundResults = response.result.results ?
                response.result.results.filter((r: FindPeerResult) => r.success).length : 0;
            ...
            // Если libp2p-node не поддерживает поиск, показываем mock результат
            await this.renderPart({
                partName: 'renderAPIResponse',
                state: {
                    responseData: {
...
/**
 * Ищет провайдеров контента в DHT через libp2p-node
 */
async function findProviders(this: DHTManager, cid: string, dhtType = 'all', maxProviders = 20): Promise<DHTResponse> {
    if (!cid || !cid.trim()) {
        await this.showModal({
            title: 'Ошибка',
    ...
    try {
        ...
        // Используем postMessage для поиска провайдеров через libp2p-node
        const response = await libp2pNode.postMessage({
            type: 'FIND_PROVIDERS',
        ...
        if (response.success && response.result) {
            ...
            // Если libp2p-node не поддерживает поиск провайдеров, показываем mock результат
...
/**
 * Публикует контент в DHT через libp2p-node
 */
async function provideContent(this: DHTManager, cid: string, dhtType = 'all'): Promise<DHTResponse> {
    if (!cid || !cid.trim()) {
        await this.showModal({
            title: 'Ошибка',
    ...
    try {
        ...
        // Используем postMessage для публикации контента через libp2p-node
        const response = await libp2pNode.postMessage({
        ...
        if (response.success && response.result) {
            ...
            const successful = response.result.results ?
                response.result.results.filter((r: any) => r.success).length : 0;
            ...
            // Если libp2p-node не поддерживает публикацию, показываем mock результат
...
/**
 * Получает информацию о бакетах DHT через libp2p-node
 */
async function getDHTBuckets(this: DHTManager, dhtType: string): Promise<DHTResponse> {
    ...
    try {
        ...
        // Используем postMessage для получения информации о бакетах
        const response = await libp2pNode.postMessage({
            type: 'GET_BUCKETS',
            data: { type: dhtType }
        });
        ...
        if (response.success) {
            ...
            // Сохраняем информацию о бакетах через renderPart
            await this.renderPart({
...
/**
 * Обновляет все данные DHT через libp2p-node
 */
async function refreshDHTData(this: DHTManager): Promise<void> {
    try {
        ...
        // Используем postMessage для обновления данных через libp2p-node
        const response = await libp2pNode.postMessage({ type: 'UPDATE_DISPLAY' });

        if (response.success) {
            // Затем загружаем обновленную статистику
            await (this as any).refreshData('all');
...
/**
 * Обновляет отображение DHT статистики
 */
async function updateDHTDisplay(this: DHTManager, data: any = null): Promise<void> {
    try {
        // Используем postMessage для обновления отображения
...
/**
 * Фильтрует статистику DHT по поисковому запросу
 */
async function filterDHTStats(this: DHTManager, searchTerm: string): Promise<void> {
    try {
        if (!searchTerm) {
            // Если поиск пустой, показываем все данные через postMessage
            await this.postMessage({ type: 'UPDATE_DISPLAY' });
            return;
        }

        // Временная реализация фильтрации
        this.showNotification(`Фильтр по запросу: \"${searchTerm}\"`, 'info');

        // Здесь можно добавить логику фильтрации по типам DHT или статусам
...
/**
 * Копирует адреса DHT в буфер обмена из libp2p-node
 */
async function copyDHTAddresses(this: DHTManager): Promise<void> {
    try {
        // Получаем libp2p-node
        const libp2pNode = await this.getComponentAsync('libp2p-node', 'libp2p-node-1');
        if (!libp2pNode) {
        ...
        // Получаем multiaddrs из libp2p-node
...
/**
 * Обрабатывает активацию вкладки DHT
 */
async function handleDHTTabActivation(this: DHTManager): Promise<void> {
    // Получаем текущее состояние через postMessage
    ...
    if (response.success && response.stats) {
        ...
        if (!hasStats) {
            // Обновляем данные через libp2p-node
            await (this as any).refreshData('all');
        }
    }
}
...
/**
 * Переключает тип отображаемой DHT статистики
 */
async function switchDHTType(this: DHTManager, dhtType: string): Promise<void> {
    try {
        // Используем postMessage для переключения типа
        const response = await this.postMessage({
            type: 'SWITCH_TYPE',
            data: { type: dhtType }
        });
...
/**
 * Скрывает контейнер с информацией о бакетах
 */
async function hideBuckets(this: DHTManager): Promise<void> {
    await this.updateElement({
        selector: '#bucketsContainer',
        value: 'hidden',
        property: 'className',
 ...
 */
// function showNotification(this: DHTManager, message: string, type = 'info'): void {
