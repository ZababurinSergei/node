...
export const controller = (context: NodeIdentity) => {
        ...
        uptimeInterval = setInterval(async () => {
            try {
                ...
                if (state.uptime && typeof state.uptime === 'string') {
                    ...
                    // Безопасное извлечение дней с использованием регулярных выражений
    ...
    const handleToggleDetails = async (): Promise<void> => {
    ...
    const setupKeyboardShortcuts = (): void => {
        addEventListener(document, 'keydown', (e: Event) => {
            ...
            // Ctrl+Alt+D для переключения деталей
    ...
    const setupExternalEvents = (): void => {
        // Слушаем события от libp2p-node
        addEventListener(window, 'libp2p:peerIdUpdated', (e: Event) => {
        ...
        addEventListener(window, 'libp2p:statusChanged', (e: Event) => {
            const customEvent = e as CustomEvent;
        ...
        // События при изменении видимости страницы
        ...
        // Глобальные команды для Node Identity
        ...
        // Событие при изменении сетевого статуса
    ...
    return {
        async init(): Promise<void> {
            ...
            // Обработка двойного клика по заголовку для обновления
            ...
            // Настройка счетчика uptime
            setupUptimeCounter();

            // Настройка горячих клавиш
            setupKeyboardShortcuts();

            // Настройка внешних событий
            setupExternalEvents();
...
export async function createActions(context: NodeIdentity) {
    return {
